...
def data_split(dataset):
    ...
    if dataset == 'Human':
        ...
        df = pd.read_table('data/metz/proteins.txt', sep=' ', header=None)
        print(len(list(df[0])))

        with open('data/metz/invalid_seq.pkl','rb') as file:
            invalid_seq = pickle.load(file)
        with open('data/metz/invalid_smile.pkl', 'rb') as file:
        ...
        for fold in range(5):
            ...
            df_test.to_csv(f'data/metz/test{fold+1}.csv',index=False, header=['compound_iso_smiles','target_sequence','affinity'])
            df_train.to_csv(f'data/metz/train{fold+1}.csv',index=False, header=['compound_iso_smiles','target_sequence','affinity'])
    else:
        print('convert data from DeepDTA for ', dataset)
        # Read the data from the raw file.
        fpath = 'data/' + dataset + '/'
        with open(fpath + "folds/train_fold_setting1.txt", 'r') as f:
            train_fold = json.load(f)
            train_fold = [int(str(ee).replace('10390', ''))  for e in train_fold for ee in e]  # 去除前缀并展平列表
        # train_fold = json.load(open(fpath + "folds/train_fold_setting1.txt"))
        # train_fold = [ee for e in train_fold for ee in e ]
        with open(fpath + "folds/test_fold_setting1.txt", 'r') as f:
            valid_fold = json.load(f)
            valid_fold = [int(str(ee).replace('10390', '')) for ee in valid_fold]
        
        #ligands = json.load(open(fpath + "ligands_can.txt"), object_pairs_hook=OrderedDict)
        with open(fpath + "ligands_can.txt") as f:
            ligands = json.load(f, object_pairs_hook=OrderedDict)
            ligands = {k.replace('10390', ''): v for k, v in ligands.items()}
        proteins = json.load(open(fpath + "proteins.txt"), object_pairs_hook=OrderedDict)
        affinity = pickle.load(open(fpath + "Y","rb"), encoding='latin1')
        print("Affinity 的形状:", affinity.shape)
        drugs = []
        prots = []
...
def construct_graph(args):
    ...
    compound_iso_smiles = []
    if args.dataset == 'Human' or args.dataset == 'metz':
        opts = ['train1', 'test1']
    else:
        opts = ['train', 'test']
    for opt in opts:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default= 'metz', help='dataset name',choices=['davis','kiba','Human','metz'])
